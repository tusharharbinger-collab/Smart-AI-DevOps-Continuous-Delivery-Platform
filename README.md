...
5. A **ChatOps query interface** — ask a project a plain-language question ("why did the last deploy roll back?") and get back an answer grounded entirely in that project's real verdicts, evidence, and audit trail (never a guess; it says so explicitly when the data doesn't cover the question), with cited run IDs you can click straight through to.
...
The **Pipeline View** tab also has a ChatOps panel — ask it things like "why did the last run fail" or "what's our rollback rate this week" and it answers from that project's real run/verdict/audit history (via Groq, with a deterministic fallback if the AI call is unavailable), never a fabricated answer.
